...
export const STROKE_OPACITY_MIN = 1;
export const STROKE_OPACITY_MAX = 100;
export const STROKE_OPACITY_STEP = 1;
...
export function normalizeStrokeOpacity(value: unknown): number {
	const numericValue = typeof value === "number" ? value : Number(value);

	if (!Number.isFinite(numericValue)) {
		return DEFAULT_STROKE_OPACITY;
	}

	const steppedValue = Math.round(numericValue / STROKE_OPACITY_STEP) * STROKE_OPACITY_STEP;
	return Math.min(STROKE_OPACITY_MAX, Math.max(STROKE_OPACITY_MIN, steppedValue));
}
...
export function normalizeDrawInCanvasSettings(
	settings: Partial<DrawInCanvasSettings> | null | undefined,
	...
	return {
		...DEFAULT_SETTINGS,
		...savedSettings,
		strokeColor: savedSettings.strokeColor?.trim() || DEFAULT_SETTINGS.strokeColor,
		strokeWidth: normalizeStrokeWidth(savedSettings.strokeWidth),
		strokeHardness: normalizeStrokeHardness(savedSettings.strokeHardness),
		strokeOpacity: normalizeStrokeOpacity(savedSettings.strokeOpacity),
		beautifulStrokes: savedSettings.beautifulStrokes === true,
		usePenCursorFallback: savedSettings.usePenCursorFallback !== false,
		allowTinyCanvasElements: savedSettings.allowTinyCanvasElements === true,
		strokeThinning: normalizeFreehandSliderValue("strokeThinning", savedSettings.strokeThinning),
		strokeStreamline: normalizeFreehandSliderValue("strokeStreamline", savedSettings.strokeStreamline),
		strokeSmoothing: normalizeFreehandSliderValue("strokeSmoothing", savedSettings.strokeSmoothing),
		strokeTaperStart: normalizeFreehandSliderValue("strokeTaperStart", savedSettings.strokeTaperStart),
		strokeTaperEnd: normalizeFreehandSliderValue("strokeTaperEnd", savedSettings.strokeTaperEnd),
	};
}
